...
bagian penting dari kontrak integrasi ini. OAuth2 bersifat opsional: bila token URL,
client ID, dan client secret diisi melalui System Parameters, wizard meminta token
`client_credentials` dan mengirimkannya melalui header `Authorization`. `sudo()`
hanya dipakai untuk membaca konfigurasi sistem, bukan untuk melewati akses user.
