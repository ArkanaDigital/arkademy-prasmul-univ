...
class AcademyApiController(http.Controller):
    ...
    def _check_api_key(self):
        # Reads X-API-Key header and compares against ir.config_parameter.
        # Parameter key : academy_management.api_key
        # Demo value    : academy-demo-key  (seeded in data/academy_api_data.xml)
        #
        # Training note: this is a simple shared-key gate — no per-user identity.
        # For production use res.users.apikeys or OAuth2.
    ...
    def _course_to_dict(self, course):
        ...
        # academy.course.description is Html — not included (not JSON-safe).
        return {
            "code": course.code or "",
            "name": course.name,
    ...
    def _course_detail_to_dict(self, course):
        # Only expose safe primitive fields.
        # academy.course.description is Html — not included (not JSON-safe).
        return {
            "code": course.code or "",
            ...
            "created_at": course.create_date.isoformat() if course.create_date else None,
            "updated_at": course.write_date.isoformat() if course.write_date else None,
            # many2many
            "tag_ids": [
                {
                    "id": tag_id.id or 0,
                ...
                for tag_id in course.tag_ids
            ] if course.tag_ids else [],
            # one2many
            "batch_ids": [
                {
                    "id": batch_id.id or 0,
                    "name": batch_id.name or "",
                }
                for batch_id in course.batch_ids
            ] if course.batch_ids else [],
        }
    ...
    def get_courses(self, **kw):
        ...
        courses = request.env["academy.course"].search([])
        return _ok([self._course_to_dict(c) for c in courses])

    # @http.route(
    #     "/academy/api/v1/courses/beginner",
    #     type="http",
    #     auth="public",
    #     methods=["GET"],
    # )
    # def get_beginner_courses(self, **kw):
    #     courses = request.env["academy.course"].sudo().get_beginner_courses()
    #     return _ok([self._course_to_dict(c) for c in courses])
    ...
    def get_course(self, code, **kw):
        ...
        course = request.env["academy.course"].sudo()\
            .get_detailed_course_by_code(code)
        if not course:
            return _err(
                "COURSE_NOT_FOUND",
        ...
        return _ok(self._course_detail_to_dict(course))
    ...
    def create_enrollment_request(self, **kw):
        # 0. Security gate — checked first, before any processing
